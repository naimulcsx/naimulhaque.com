...
<Callout>
  If you are coming from a different language e.g. C++, Java, C# etc, where the
  `this` keyword represents the current instance of a class, I've a bad news for
  you. JavaScript's `this` keyword is not the same. So it's better not to
  compare with the languages you already know.
</Callout>
...
When we use the `this` keyword in the global scope (i.e. outside any function), it's value refers to the _global object_.
...
Global functions like `console.log`, `parseInt`, `setTimeout` or global objects like `Date` are properties of the `window` object.
...
When a method is called on an object in JavaScript, the `this` keyword inside that method refers to the object itself. This behavior is known as _method binding_ and it's a fundamental aspect of how object-oriented programming works in JavaScript. Let's look at the following example
